    ...
    static func getMessages(for channel: ChannelItem, completion: @escaping ([MessageItem]) -> Void) {
        FirebaseConstants.MessagesRef.child(channel.id).observe(.value) { snapshot in
            ...
            dict.forEach { key, value in
                ...
                if let thumbnailUrl = messageDict["thumbnailUrl"] as? String {
                    print("Message thumbnail URL: \(thumbnailUrl)")
                }
                if let audioURL = messageDict["audioURL"] as? String {
                    print("Message audio URL: \(audioURL)")
                }
                if let videoURL = messageDict["videoURL"] as? String {
                    print("Message video URL: \(videoURL)")
                }
